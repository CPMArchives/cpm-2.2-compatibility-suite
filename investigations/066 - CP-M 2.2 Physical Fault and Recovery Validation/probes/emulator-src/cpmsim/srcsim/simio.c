...
/* Investigation 066 isolated extension of the I015 selector: value 3 performs
 * the next physical write and then reports write failure. */
static BYTE inv015_fault;
...
static void inv015_fault_out(BYTE data);
...
static void fdco_out(BYTE data)
{
	...
	off_t pos;
	static char buf[128];
	int inv066_post_write = (inv015_fault == 3 && data == 1);

	if ((inv015_fault == 1 && data == 0) ||
	    (inv015_fault == 2 && data == 1)) {
		status = (data == 0) ? 5 : 6;
		inv015_fault = 0;
		return;
	}
	...
	case 1:	/* write */
		...
		if (write(*disks[drive].fd, buf, 128) != 128)
			...
			status = inv066_post_write ? 6 : 0;
		break;
	default:		/* invalid command */
		status = 7;
		break;
	}
	if (inv066_post_write)
		inv015_fault = 0;
}
