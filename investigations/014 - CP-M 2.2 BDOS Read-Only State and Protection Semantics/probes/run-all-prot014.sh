...
set -e
here=${0:A:h}
base="$here/staging-investigation-014/probes/images-before/drivea.dsk"
rundir=/private/tmp/inv014-runs
rm -rf "$rundir"
mkdir -p "$rundir"
for mode in N A B C D E F G H I J K; do
  mkdir -p "$rundir/$mode"
  cp "$base" "$rundir/$mode/drivea.dsk"
  shasum -a 256 "$rundir/$mode/drivea.dsk" > "$rundir/$mode/before.sha256"
  "$here/run-prot014-case.exp" "$mode" "$rundir/$mode" "$rundir/$mode/console.txt"
